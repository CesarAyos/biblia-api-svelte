<script lang="ts">
	...
	} from '../api/fetchData'; // Importa las funciones necesarias desde el archivo correspondiente
	import type { Version, Book, SearchData } from '../api/fetchData';
	import Chapter from '../components/Chapter.svelte'; // Importa tu componente de capítulo (si es necesario)
	...
	let loadingSearch = false; // Estado de carga de búsqueda
	let currentPage = 0; // Página actual de búsqueda
	let progress = 0; // Progreso inicial de descarga
	let loading = false; // Estado de carga general
